...
Avo.configure do |config|
  ...
  config.authenticate_with do
    redirect_to main_app.root_path, alert: "Acesso não autorizado." unless current_user&.admin?
  end

  ## == Authorization ==
  # Pundit integration (`authorization_client = :pundit`) requires the
  # `avo-authorization` plugin, which is part of Avo's paid tier (not on
  # public RubyGems) — not installed here. The `authenticate_with` block
  # above is the only gate: it fully blocks every non-admin before they
  # reach any resource, which is sufficient since admin already bypasses
  # every domain policy elsewhere in the app. `RolePolicy`/`UserPolicy`/
  # `DriverTruckPolicy` and `ApplicationPolicy`'s `avo_*` methods exist
  # and are ready to use, but are currently dormant — Avo never calls
  # them without the plugin. Revisit if the license is ever purchased.
